...
def load_app_creds(creds_path: Path) -> dict:
    with creds_path.open(mode='r', encoding='utf8') as \
            credentials_stream:
        return json.load(credentials_stream)


def load_user_token(ut_path: Path) -> Optional[UserCredentials]:
    with ut_path.open(mode='r', encoding='utf8') as \
            token_stream:
        ...
        try:
            ...
            logging.exception('RefreshError occurred while refreshing ' +
                              'user token. Most likely the user\'s ' +
...
def load_service_account(sa_path: Path) -> \
...
def save_user_token(user_creds: UserCredentials,
                    user_token_path: Path) -> NoReturn:
...
def revoke_token(token: str) -> bool:
    response = request('POST', 'https://oauth2.googleapis.com/revoke',
                       params={'token': token},
...
def auth_new_user(app_creds: dict, scopes: Sequence[str],
                  headless: bool = False) -> Optional[UserCredentials]:
    flow = InstalledAppFlow.from_client_config(app_creds, scopes)

    try:
        return flow.run_console() if headless else \
