...
class TableModel(QAbstractTableModel):
    ...
    def update(self, dataIn):
        printer.debug('TableModel update start')
        self.datatable = []

        for value in dataIn.values():
            new_item = {}
            new_item[Column.FILENAME] = value.fileName()
        ...
        self._sort()
        printer.debug('TableModel update finished')

    def rowCount(self, parent=QtCore.QModelIndex()):
        printer.debug('TableModel rowCount')
        return len(self.datatable)

    def columnCount(self, parent=QtCore.QModelIndex()):
        printer.debug('TableModel columnCount')
        return self.column_count

    def data(self, index, role=Qt.DisplayRole):
        printer.debug('TableModel data')
    ...
    def flags(self, index):
        printer.debug('TableModel flags')
    ...
    def _sort(self):
        ...
        self.datatable.sort(key=lambda item: item[Column(self.sort_column)], reverse=self.sort_order != SortDirection.ASC)
