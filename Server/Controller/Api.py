		...
		with open(path, "rb") as f:
			...
			if start and end:
				...
				if start >= size or start < 0 or end <= 0:
					return Server.Response400(None, response, 'Invalid range request %d - %d' % (start, end)) # request was unavailable and isnt checked by Response400
...
def getDirectoryList2(request, response):
	path = ''
	for i in request.bits[2:]:
		path = os.path.join(path, i)
